...
#include "Player.h"
#include <iostream>
class P2_be : public Behaviour {

public:
	P2_be():Behaviour{}{}

    void process(World &world, Entity& owner, sf::Time const& t){
        Player& o = dynamic_cast<Player&>(owner);
        sf::Vector2f dir = world.player2.direction();
    ...
    void shoot(World& world, Player& owner, sf::Time t)const{
        if (world.player2.shoot && owner.time_since_last_shot + t.asSeconds() > 0.5f){
            world.add_entity(
    ...
    void move_y(Entity& owner, sf::Vector2f const& dir, sf::Time t){
        if (velocity.y == 0.0f){
    ...
    void move_x(Entity& owner, sf::Vector2f const& dir, sf::Time t){
    ...
    void collision_y(World& world, Entity& owner){
    ...
    void collision_x(World& world, Entity& owner){
