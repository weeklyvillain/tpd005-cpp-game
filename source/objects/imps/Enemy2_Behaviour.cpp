#include "../headers/Enemy2_Behaviour.h"
...
void Enemy2_Behaviour::process(World& world, Entity& owner, sf::Time const& t){
    ...
    if (direction < 0){
        dir.x = 1.0f;
        if (owner.getPosition().x > end_pos.x){
            sf::Vector2f tmp{end_pos};
            end_pos = start_pos;
            start_pos = tmp;
        }
    }else{
...
void Enemy2_Behaviour::flip(Entity& owner, sf::Vector2f const& dir){
...
void Enemy2_Behaviour::move_y(Entity& owner, sf::Vector2f const& dir, sf::Time t){
    if (velocity.y == 0.0f){
        velocity.y = dir.y * 7;
    }
